...
//import DatePicker from '../Filters/Date'
...
const View = (props) => {
  ...
  const fillDate = async (batch) => {
    const result = (await axios.get("/report_date/2") ).data
    if(result.length != 0){
      setRptDate(new Date(result[0].rpt2_date))
      fillData()
    }else{
      setDataSource([])
    }
  ...
  const fillData = async (batch) => {
    ...
    /*if(DateFilter !== undefined){
      if(DateFilter.indexOf("-") !== -1){
        const dateData = DateFilter.split("-");
        filters.Year = dateData[0]
        filters.Month = dateData[1]
      }
    }*/
    
    try {
      const result = (await axios.post("/report_2", filters) ).data
      if(result != "ERROR" ){
        grandtotal_avg_sales = 0
        grandtotal_first_week = 0
        grandtotal_scope= 0

        setDataSource((result == "Not Data" ? [] : result.map((el, i) => {
          grandtotal_avg_sales += Math.round(el.rpt2_avg_sales_weekly);
          grandtotal_first_week += Math.round(el.rpt2_first_week);
          grandtotal_scope += Math.round(el.rpt2_scope);
    
            return {
              ...el,
              key: i,
              group: (el.rpt2_group).toUpperCase(),
              averageSales: Math.round(el.rpt2_avg_sales_weekly).toLocaleString('en-EN'),
              firstWeek:Math.round(el.rpt2_first_week).toLocaleString('en-EN'),
              scope: (el.rpt2_scope === null ? 0 : parseFloat(el.rpt2_scope).toLocaleString('en-EN', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '%')   
            }
          })
        ))
        if(result.length !== 0 || result == "Not Data" || result == null){
          SetTotales({//Totales      
              ...
              firstWeek: Math.round(grandtotal_first_week).toLocaleString('en-EN'),
  ...
  const exportPDF = () => {
    ...
    const title = "Cliente con Vtas de la Sem 1 por debajo del Promedio de Vtas Semanal";
    const headers = [
      { header: 'Agrupación', dataKey: 'group' },
      { header: 'Promedio de Ventas', dataKey: 'averageSales',  },
      { header: 'Ventas Semana 1', dataKey: 'firstWeek' },
    ...
    dataS = [ ...dataSource.map( value => { 
      return {
        ...
        scope: parseFloat(value.scope).toLocaleString('en-EN', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + '%'
      }
    }), 
      {//Totales
        averageSales: Totales.averageSales,
        firstWeek:Totales.firstWeek,
        scope: Totales.scope 
      }]
...
for (let j = 1; j < pages + 1 ; j++) {
  ...
  const footer = () => {
    return(
      ...
      <Col xs={0} md={0} lg={6} xl={6} style={{ textAlign: 'right', fontWeight: "bold", paddingRight:"23px" }} > {Totales.firstWeek}</Col>
      ...
      <Col xs={24} md={24} lg={0} xl={0} style={{ textAlign: 'right', fontWeight: "bold" }} >Vts Semana 1: {Totales.firstWeek}</Col>
    ...
    <div style={{paddingBottom: "50px"}}>
        ...
        <Row>
          <Col xs={24} md={24} lg={24} xl={24}>
            <Title level={4}>Clientes con ventas  en la semana uno por debajo del promedio de ventas semanal</Title>           
          </Col>
